...
def locate_plate(orig_img, pred_image):
    ...
    cv2.imwrite('./cv-cnn-lpr-main/data/output/plate_locate.jpg', temp1_orig_img)
...
def get_chars(car_plate_char):
    ...
    cv2.imwrite('./cv-cnn-lpr-main/data/output/plate_bin.jpg', plates)
    ...
    for i, addr in enumerate(char_addr_list):
        ...
        cv2.imwrite('./cv-cnn-lpr-main/data/output/char_' + str(i + 1) + '.jpg', char_img)
...
if __name__ == '__main__':
    ...
    char_w, char_h = 20, 20
    # 训练好的模型路径，名称要核对好
    plate_model_path = os.path.join('./cv-cnn-lpr-main/data/model/plate_recognize/model.ckpt-510')
    char_model_path = os.path.join('./cv-cnn-lpr-main/data/model/char_recognize/model.ckpt-600')

    img = cv2.imread('./cv-cnn-lpr-main/data/testset/set1/21.jpg')
    ...
    if not ret:
        print("未检测到车牌")
    #cv2.imshow('cnn_plate_img', ret)
    #cv2.imwrite('./cv-cnn-lpr-main/data/output/plate_cnn.jpg', cnn_plate)
